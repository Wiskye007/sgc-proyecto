            ...
            <div className="container mx-auto max-w-7xl relative z-10 space-y-8">
                ...
                <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 bg-card/80 p-6 rounded-2xl border border-border/80 backdrop-blur-xl shadow-2xl">
                    <div className="flex items-center gap-5">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h1 className="text-3xl font-black tracking-wide text-foreground">Panel de Reportes</h1>
                                <p className="text-primary text-xs font-bold uppercase tracking-widest mt-1">Estadísticas y Analíticas del Sistema</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <Card className="sgc-card border-0 shadow-2xl">
                    <CardHeader className="border-b border-border/60 pb-4">
                        <CardTitle className="text-xl text-foreground">Visualización de datos en tiempo real</CardTitle>
                        <CardDescription className="text-foreground text-[15px]">Resumen consolidado de la población y estado de la carceleta</CardDescription>
                    </CardHeader>
                        ...
                        {chartsLoading ? (
                            <div className="flex items-center justify-center py-12">
                                <div className="text-center space-y-3">
                                    ...
                                    <p className="text-foreground">Cargando gráficos...</p>
                                </div>
                            </div>
                        ) : (
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                ...
                                <div className="bg-card/50 rounded-xl p-5 border border-border/80 shadow-inner">
                                    <h3 className="text-xs font-bold tracking-widest text-foreground uppercase mb-6 flex items-center gap-2">
                                        <Users className="h-4 w-4 text-primary"/> Ocupación por Pabellón
                                    </h3>
                                    {datoPoblacionPabellones.length > 0 ? (
                                        ...
                                        <div className="h-70 flex items-center justify-center text-foreground">
                                                ...
                                                return (
                                                    <div key={idx}>
                                                        <div className="flex justify-between text-sm text-foreground mb-1">
                                                            <span className="text-sm font-medium text-foreground">{pab.nombre}</span>
                                            ...
                                            <div className="mt-4 flex flex-wrap gap-4 justify-center">
                                                {datoPrioridades.map((item, idx) => (
                                                    <div key={idx} className="flex items-center gap-2">
                                                        ...
                                                        <span className="text-sm text-foreground">{item.prioridad}</span>
                                                    </div>
                                                ))}
                                            </div>
                                        </div>
                ...
                <Card className="sgc-card border-0 shadow-2xl">
                    <CardHeader className="border-b border-border/60 pb-4">
                        ...
                        <CardDescription className="text-foreground text-[15px]">Selecciona el módulo de datos que deseas procesar</CardDescription>
                    </CardHeader>
                    <CardContent className="pt-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                            {tiposReporte.map((tipo) => (
                                <Card
                                    key={tipo.id}
                                    className="bg-card/60 border border-border/80 hover:border-blue-500/50 hover:bg-slate-800/40 cursor-pointer transition-all duration-300 group hover:-translate-y-1 hover:shadow-[0_0_20px_rgba(59,130,246,0.15)]"
                                    onClick={() => handleSelectReportType(tipo.id)}
                                >
                                    <CardHeader className="pb-3">
                                        <div className="flex items-start gap-4">
                                            <div className="rounded-xl bg-card p-3 border border-border group-hover:bg-blue-500/20 group-hover:border-blue-500/30 transition-colors shadow-inner">
                                                <FileText className="h-6 w-6 text-primary"/>
                                            </div>
                                            <div>
                                                <CardTitle className="text-base text-foreground group-hover:text-foreground transition-colors">{tipo.nombre}</CardTitle>
                                                <CardDescription className="text-xs text-foreground mt-1">{tipo.descripcion}</CardDescription>
                                            </div>
                                        </div>
                                    </CardHeader>
                                    <CardContent>
                                        <div className="w-full h-9 rounded-lg flex items-center justify-center gap-2 bg-slate-800/50 text-foreground font-semibold border border-slate-700/50 group-hover:bg-primary group-hover:text-foreground group-hover:border-transparent transition-all duration-300 text-sm">
                ...
                <Card className="sgc-card border-0 shadow-2xl">
                    <CardHeader className="border-b border-border/60 pb-4">
                        ...
                        <CardDescription className="text-foreground text-[15px]">Documentos generados y almacenados en la base de datos</CardDescription>
                    </CardHeader>
                    <CardContent className="pt-6">
                        {loading ? (
                            <div className="text-center py-12 text-foreground animate-pulse">Consultando base de datos...</div>
                        ) : savedReports.length === 0 ? (
                            <div className="text-center py-12 bg-card/40 rounded-xl border border-dashed border-border text-foreground">
                                No se encontraron documentos. Genere un nuevo reporte para visualizarlo aquí.
                            </div>
                        ) : (
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                                {savedReports.map((report) => (
                                    <Card key={report.id} className="bg-card border border-border shadow-lg hover:border-slate-600 transition-all">
                                        <CardHeader className="pb-3">
                                            <CardTitle className="text-base text-foreground line-clamp-2 leading-snug">{report.asunto}</CardTitle>
                                            <CardDescription className="text-xs text-primary/80 font-medium mt-1">
                                                {report.fecha} <span className="text-foreground mx-1">|</span> {report.tipoReporte.toUpperCase()}
                                            </CardDescription>
                                        </CardHeader>
                                        <CardContent>
                                            <div className="flex gap-2">
                                                <Button size="sm" className="flex-1 bg-slate-800 hover:bg-slate-700 text-foreground border-0 h-9"
                                                    onClick={() => { setSelectedReport(report); setOpenViewDialog(true); }}>
                                                    <Eye className="h-3.5 w-3.5 mr-1.5 text-primary"/> Ver
                                                </Button>
                ...
                <Dialog open={openNewDialog} onOpenChange={setOpenNewDialog}>
                    <DialogContent className="sgc-card border-border text-foreground sm:max-w-2xl max-h-[90vh] overflow-y-auto">
                        <DialogHeader className="border-b border-border/80 pb-4">
                            ...
                            <DialogDescription className="text-foreground text-xs mt-1">Complete los parámetros requeridos para la extracción de datos.</DialogDescription>
                        </DialogHeader>

                        <div className="space-y-6 pt-2">
                            <div className="space-y-4">
                                <h3 className="font-bold tracking-widest text-foreground uppercase text-xs">I. Parámetros del Documento</h3>
                                    ...
                                    <div className="space-y-1.5">
                                        ...
                                        <Select value={formData.periodo} onValueChange={(v) => updateFormField("periodo", v)}>
                                            ...
                                            <SelectContent className="bg-[#111827] border border-border text-foreground">
                                                <SelectItem value="actual" className="focus:bg-blue-600 focus:text-foreground">Estado Actual</SelectItem>
                                                <SelectItem value="mes" className="focus:bg-blue-600 focus:text-foreground">Este mes</SelectItem>
                                                <SelectItem value="trimestre" className="focus:bg-blue-600 focus:text-foreground">Este trimestre</SelectItem>
                                                <SelectItem value="anio" className="focus:bg-blue-600 focus:text-foreground">Este año</SelectItem>
                                            </SelectContent>
                                        </Select>
                                    </div>
                                </div>
                            ...
                            <div className="space-y-4">
                                <h3 className="font-bold tracking-widest text-foreground uppercase text-xs">II. Firmas y Responsables</h3>
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-1.5">
                            ...
                            <div className="space-y-4">
                                <h3 className="font-bold tracking-widest text-foreground uppercase text-xs">III. Notas Adicionales</h3>
                            ...
                            <div className="space-y-1.5">
                                ...
                                <Select value={formData.formato} onValueChange={(v) => updateFormField("formato", v)}>
                                    ...
                                    <SelectContent className="bg-[#111827] border border-border text-foreground">
                                        <SelectItem value="pdf" className="focus:bg-blue-600 focus:text-foreground">Documento PDF (.pdf)</SelectItem>
                                        <SelectItem value="excel" className="focus:bg-blue-600 focus:text-foreground">Hoja de Cálculo (.xlsx)</SelectItem>
                                        <SelectItem value="csv" className="focus:bg-blue-600 focus:text-foreground">Valores por Comas (.csv)</SelectItem>
                                    </SelectContent>
                                </Select>
                            </div>
                ...
                <Dialog open={openViewDialog} onOpenChange={setOpenViewDialog}>
                    <DialogContent className="sgc-card border-border text-foreground sm:max-w-2xl max-h-[90vh] overflow-y-auto">
                        <DialogHeader className="border-b border-border/80 pb-4">
                            <DialogTitle className="text-xl font-bold text-foreground pr-6 leading-tight">{selectedReport?.asunto}</DialogTitle>
                            <DialogDescription className="text-primary font-mono text-xs mt-1">EMITIDO: {selectedReport?.fecha}</DialogDescription>
                        </DialogHeader>

                        {selectedReport && (
                            <div className="space-y-6 pt-2">
                                <div className="grid grid-cols-2 gap-4 bg-card/50 p-4 rounded-xl border border-border">
                                    <div>
                                        <Label className="text-[10px] uppercase tracking-widest text-foreground font-bold mb-1 block">Tipo de Reporte</Label>
                                        <p className="text-sm font-medium text-foreground">{selectedReport.tipoReporte.toUpperCase()}</p>
                                    </div>
                                    <div>
                                        <Label className="text-[10px] uppercase tracking-widest text-foreground font-bold mb-1 block">Formato Base</Label>
                                        <Badge variant="outline" className="bg-slate-800 text-foreground border-slate-700">{selectedReport.formato.toUpperCase()}</Badge>
                                    </div>
                                    </div>
                                <div>
                                    <Label className="text-[10px] uppercase tracking-widest text-foreground font-bold mb-2 block">Observaciones Adjuntas</Label>
                                    <div className="bg-card p-4 rounded-lg border border-border/80 text-sm text-foreground min-w-20">
                                        {selectedReport.datos.observaciones || <span className="text-foreground italic">No se registraron observaciones.</span>}
                                    </div>
                                </div>
                                ...
                                <div className="flex flex-wrap gap-2 pt-4 border-t border-border/80">
                                    ...
                                    <Button onClick={handleEliminarReporte} className="bg-red-500/10 text-red-400 hover:bg-red-600 hover:text-foreground border border-red-500/30 transition-colors h-11 px-4 rounded-md">
